...
class PFLD_GhostNet(Module):
    def __init__(self, width_factor=1, input_size=112, landmark_number=98):
        ...
        #  has_bn=False 这个参数通常用于控制是否在卷积层后添加批量归一化（Batch Normalization）层。False 表示不添加 Batch Normalization 层。
    ...
    def forward(self, x):
        x = self.conv1(x)#                    input:112*112*3  out:56*56*64
        x = self.conv2(x)#                    out:56*56*64
        x1 = self.avg_pool1(x)#               out:56 56 64
        #  print(x1.shape)
        x1 = x1.view(x1.size(0), -1)
        #  print(x1.shape)

        x = self.conv3_1(x)#                  input:56*56*64   out:28*28*80
        x = self.conv3_2(x)#                  out:28*28*80
        x = self.conv3_3(x)#                  out:28*28*80
        x2 = self.avg_pool2(x)#               out:28*28*80
        x2 = x2.view(x2.size(0), -1)


        x = self.conv4_1(x)#                  input:28*28*80   out:14*14*96
        x = self.conv4_2(x)#                  out:14*14*96
        x = self.conv4_3(x)#                  out:14*14*96
        x3 = self.avg_pool3(x)#               out:14*14*96
        x3 = x3.view(x3.size(0), -1)

        x = self.conv5_1(x)#                  input:14*14*96   out:7*7*144
        x = self.conv5_2(x)#                  out:7*7*144
        x = self.conv5_3(x)#                  out:7*7*144
        x = self.conv5_4(x)#                  out:7*7*144
        x4 = self.avg_pool4(x)#               out:7*7*144
        x4 = x4.view(x4.size(0), -1)

        x = self.conv6(x)#                  input:7*7*144   out:7*7*16
        x = self.conv7(x)#                  input:7*7*16    out:7*7*32
        x5 = self.conv8(x)#                 input:7*7*32    out:**128  kerl = 7*7
